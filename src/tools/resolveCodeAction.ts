...
export async function resolveCachedCodeAction(cached: CachedAction) {
  try {
    ...
    const refreshedActions = (await cached.client.codeActions(
      cached.file,
      cached.range,
    )) as CodeActionOrCommand[];
    const refreshedAction = findMatchingCodeAction(
      refreshedActions,
      cached.action,
    );
    if (!refreshedAction) {
      throw new Error(
        `Cached code action is stale and no matching action was returned by Roslyn: ${getErrorMessage(error)}`,
      );
    }
